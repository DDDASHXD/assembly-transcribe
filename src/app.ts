...
export async function main() {
    ...
    options: [
      {
        ...
        value: ['universal-3-pro', 'universal-2'],
      },
  ...
  renderer.keyInput.on('keypress', (key) => {
    ...
    if (!modal.visible && activeJob?.utterances?.length && !isTextInputFocused()) {
      if (key.name === 'n') {
        selectedUtteranceIndex = Math.min(
          selectedUtteranceIndex + 1,
          activeJob.utterances.length - 1,
        )
  ...
  languageSelect.on(
    SelectRenderableEvents.ITEM_SELECTED,
    (_index: number, option: { value?: boolean }) => {
  ...
  diarizationSelect.on(
    SelectRenderableEvents.ITEM_SELECTED,
    (_index: number, option: { value?: boolean }) => {
      settings.speakerLabels = option.value ?? true
      setStatus(
        settings.speakerLabels ? 'Speaker diarization enabled.' : 'Speaker diarization disabled.',
      )
    },
  )

  speakerCountSelect.on(
    SelectRenderableEvents.ITEM_SELECTED,
    (_index: number, option: { value?: number }) => {
      settings.maxSpeakers = option.value
      setStatus(
        settings.maxSpeakers
          ? `Expecting ${settings.maxSpeakers} speakers.`
          : 'Speaker count set to auto.',
      )
    },
  )
  ...
  input.on(InputRenderableEvents.CHANGE, async () => {
    ...
    const allValid = (await Promise.all(resolved.map((p) => validateAudioFile(p)))).every(
      (v) => v === null,
    )
    if (!allValid) return
  ...
  function updateJob(filePath: string, patch: Partial<Job>) {
    const job = jobs.find(
      (item) => item.filePath === filePath && item.status !== 'done' && item.status !== 'error',
    )
    if (!job) return
  ...
  async function transcribeFile(filePath: string) {
    ...
    try {
      ...
      const completedJob = jobs.find(
        (item) => item.filePath === filePath && item.status === 'transcribing',
      )
      if (completedJob) {
      ...
      const outputPath = await writeTranscript(
        filePath,
        formatTranscript(result.text || '', result.utterances ?? undefined),
      )
      updateJob(filePath, {
        status: 'done',
  ...
  function getAudioDurationMs() {
    return (
      activeJob?.utterances?.reduce((max, utterance) => Math.max(max, utterance.end ?? 0), 0) ?? 0
    )
  }
  ...
  function createTranscriptRow(index: number, speaker: string | null | undefined, text: string) {
    ...
    return new TextRenderable(renderer, {
      ...
      onMouseDown: (event: { x: number; button: number }) => {
        ...
        const isDoubleClick =
          lastTranscriptClick?.index === index && now - lastTranscriptClick.time < 450
  ...
  async function saveInlineEdit(index: number, value: string) {
    ...
    activeJob!.text =
      activeJob!.utterances?.map((item) => item.text ?? '').join(' ') ?? activeJob!.text
